...
export class IntegrationManager {
  ...
  async saveVapiConfig(
    ...
    try {
      ...
      if (!config.apiKey || !config.assistantId) {
        return {
          success: false,
          error: "Missing required fields: apiKey and assistantId are required",
        };
      }

      // Map camelCase to snake_case for database and handle empty strings
      const dbConfig = {
      ...
      console.log("2. Mapped dbConfig:", {
        ...dbConfig,
        api_key: dbConfig.api_key ? "***" : "undefined",
      });
      ...
      if (!data || !data.api_key || !data.assistant_id) {
        console.error("VAPI config saved but missing required fields:", data);
        return {
          success: false,
          error: "Configuration saved but missing required fields",
        };
      }
    ...
    try {
      ...
      if (!config.accountSid || !config.authToken || !config.phoneNumber) {
        return {
          success: false,
          error:
            "Missing required fields: accountSid, authToken, and phoneNumber are required",
        };
      }
    ...
    try {
      ...
      if (!config.apiKey || !config.fromEmail || !config.fromName) {
        return {
          success: false,
          error:
            "Missing required fields: apiKey, fromEmail, and fromName are required",
        };
      }
      ...
      if (!data || !data.api_key || !data.from_email || !data.from_name) {
        console.error(
          "SendGrid config saved but missing required fields:",
          data,
        );
        return {
          success: false,
  ...
  private async ensureStatusEntriesExist(): Promise<void> {
    try {
      ...
      for (const type of integrationTypes) {
        await this.supabase.from("integration_statuses").upsert(
          {
            user_id: this.userId,
  ...
  async updateIntegrationStatus(
    ...
    testStatus: "success" | "failed" | "never" = "never",
  ): Promise<void> {
    try {
      ...
      const { data, error } = await this.supabase
        ...
        .upsert({
          ...
          last_tested_at:
            testStatus !== "never" ? new Date().toISOString() : null,
          updated_at: new Date().toISOString(),
        })
        .select();
  ...
  async getAllConfigurations() {
    try {
      const [vapiConfig, twilioConfig, sendGridConfig, statuses] =
        await Promise.all([
          this.getVapiConfig(),
