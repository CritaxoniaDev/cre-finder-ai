  ...
  if (subscription.status === "active" || subscription.status === "trialing") {
    ...
    if (priceData?.product_id) {
      ...
      const maxSearches = metadata
        ...
        : 0;
      if (maxSearches > 0) {
        // If subscription is trialing, only give 5 credits
        const creditAmount =
          subscription.status === "trialing" ? 5 : maxSearches;
        ...
        console.log(
          `Added ${creditAmount} subscription credits for user [${userId}] (status: ${subscription.status})`,
        );
      }
    }
  }
