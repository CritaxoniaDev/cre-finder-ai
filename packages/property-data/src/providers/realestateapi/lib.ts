...
export async function getPropertySearch(
  ...
  let resultIndex = 0;
  const pageSize = transformedParams.size || 250; // Use provided size or default to 250
  let totalRecords = 0;
  let firstResponse: PropertySearchResponse | null = null;

  // If a specific size is provided, limit the total records to that size
  const maxRecords = transformedParams.size || Number.MAX_SAFE_INTEGER;
  ...
  do {
    const response = await fetch(
      "https://api.realestateapi.com/v2/PropertySearch",
      {
        ...
        body: JSON.stringify({
          ...transformedParams,
          count: false,
          size: Math.min(pageSize, maxRecords - allResults.length), // Don't fetch more than needed
          resultIndex,
        }),
      },
    );
    ...
    resultIndex < totalRecords &&
    allResults.length < totalRecords &&
