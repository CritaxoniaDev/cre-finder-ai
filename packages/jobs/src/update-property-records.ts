...
export const updatePropertyRecordsTask = schemaTask({
  ...
  maxDuration: 300,
  run: async (payload) => {
    const { licenseId, useTestData } = payload;
    const supabase = createClient();

    const { data: license } = await getLicenseLocationByIdQuery(
      supabase,
      licenseId,
    );
    ...
    const { data: asset } = await getAssetTypeQuery(
      supabase,
      license.asset_licenses.asset_type_slug,
    );
    ...
    // Check user's subscription status to determine if trial size limit should be applied
    const { data: subscription } = await supabase
      .from("subscriptions")
      .select("status")
      .eq("user_id", assetLicense.user_id)
    ...
    if (useTestData) {
      ...
      // Prepare search params, applying trial size limit if user is on trial
      const originalSearchParams =
        assetLicense.search_params as unknown as GetPropertySearchParams;
      const searchParams = isTrialUser
        ? { ...originalSearchParams, size: TRIAL_SIZE_LIMIT }
        : originalSearchParams;

      if (isTrialUser) {
        logger.info(
          `Trial user detected for license [${license.id}]. Search params:`,
          { searchParams, trialSizeLimit: TRIAL_SIZE_LIMIT },
        );
      }
      ...
      logger.info(
        `Fetched ${propertyData.length} properties in ${executionTime}ms for license [${license.id}] for location [${license.location_internal_id}]${isTrialUser ? ` (trial mode - limited to ${TRIAL_SIZE_LIMIT} results)` : ""}`,
      );
    }
    ...
    // Only trigger skip trace for real data, not test data, and not for trial users
    if (!useTestData && !isTrialUser) {
      ...
      if (isTrialUser) {
        logger.info("Skipping skip trace task for trial user");
      }
    }
