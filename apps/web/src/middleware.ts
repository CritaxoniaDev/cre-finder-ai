...
export async function middleware(request: NextRequest) {
  ...
  if (!user && !isAuthPage && newUrl.pathname !== "/") {
    ...
    if (pathnameWithoutLocale !== "/") {
      ...
      redirectUrl.searchParams.append("return_to", "/dashboard");
    }

    // Pass original search params as-is when coming from marketing page
    if (isFromMarketing && nextUrl.searchParams.size > 0) {
