...
const searchParamsSchema = z.object({
  ...
  filters: z
    .object({
      ...
      price_type: z
        .array(z.enum(["sale", "rent", "lease", "auction", "negotiable"]))
        .optional(),
    ...
    .optional(),
  sort_by: z
    .enum(["price", "created_at", "updated_at", "square_feet", "bedrooms"])
    .optional(),
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (minPrice) {
      filters.min_price = Number.parseFloat(minPrice);
    }

    const maxPrice = searchParams.get("max_price");
    if (maxPrice) {
      filters.max_price = Number.parseFloat(maxPrice);
    }
    ...
    if (maxBedrooms) {
      filters.max_bedrooms = Number.parseInt(maxBedrooms);
    }

    // Bathroom filters
    const minBathrooms = searchParams.get("min_bathrooms");
    if (minBathrooms) {
      filters.min_bathrooms = Number.parseFloat(minBathrooms);
    }

    const maxBathrooms = searchParams.get("max_bathrooms");
    if (maxBathrooms) {
      filters.max_bathrooms = Number.parseFloat(maxBathrooms);
    }

    // Square feet filters
    const minSquareFeet = searchParams.get("min_square_feet");
    if (minSquareFeet) {
      filters.min_square_feet = Number.parseInt(minSquareFeet);
    }

    const maxSquareFeet = searchParams.get("max_square_feet");
    if (maxSquareFeet) {
      filters.max_square_feet = Number.parseInt(maxSquareFeet);
    }
