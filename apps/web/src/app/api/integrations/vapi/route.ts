...
export async function GET(request: NextRequest) {
  try {
    ...
    const safeConfig = {
      ...
      assistantId: config.assistant_id ? decrypt(config.assistant_id) : "",
      organization: config.organization || "",
      phoneNumber: config.phone_number ? decrypt(config.phone_number) : "",
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Use phone number from request body if provided, otherwise try to fetch from Twilio
    ...
    const configToSave = {
      apiKey: encrypt(providedApiKey),
      assistantId: encrypt(String(body.assistantId)),
      organization,
