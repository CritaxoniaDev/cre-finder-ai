...
export async function testTwilioConfigurationAction(
  config: z.infer<typeof twilioConfigSchema>,
) {
  try {
    ...
    if (upsertError) {
      ...
      if (checkError && checkError.code !== "PGRST116") {
        ...
        throw new Error(
          `Failed to check existing Twilio configuration: ${checkError.message}`,
        );
      }
      ...
      if (existingConfig) {
        ...
        if (updateError) {
          ...
          throw new Error(
            `Failed to update Twilio configuration: ${updateError.message}`,
          );
        }
        ...
        if (insertError) {
          ...
          throw new Error(
            `Failed to save Twilio configuration: ${insertError.message}`,
          );
        }
    ...
    throw new Error(
      `Twilio configuration test failed: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
...
export async function getTwilioConfigurationAction() {
  try {
    ...
    throw new Error(
      `Failed to get Twilio configuration: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
...
export async function testTwilioSMSAction(
  ...
  try {
    ...
    if (configError || !config) {
      throw new Error(
        "Twilio configuration not found. Please configure Twilio first.",
      );
    }
    ...
    throw new Error(
      `Twilio SMS test failed: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
...
export async function testTwilioVoiceAction(phoneNumber: string) {
  try {
    ...
    throw new Error(
      `Twilio Voice test failed: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
