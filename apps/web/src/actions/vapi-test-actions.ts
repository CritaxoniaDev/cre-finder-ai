...
const testPropertySchema = z.object({
  ...
  price_type: z
    .enum(["sale", "rent", "lease", "auction", "negotiable"])
    .default("sale"),
...
async function createUserThroughAuth(
  email: string,
  password: string,
  userData: any,
) {
  try {
    ...
    if (publicUserError) {
      throw new Error(
        `Public user creation failed: ${publicUserError.message}`,
      );
    }
...
async function saveVapiConfigWithFallback(
  ...
  try {
    console.log(
      "Attempting VAPI config save with fallback approach for user:",
      userId,
    );
    ...
    if (checkError && checkError.code !== "PGRST116") {
      ...
      throw new Error(
        `Failed to check existing VAPI configuration: ${checkError.message}`,
      );
    }
    ...
    if (existingConfig) {
      ...
      if (updateError) {
        ...
        throw new Error(
          `Failed to update VAPI configuration: ${updateError.message}`,
        );
      }
    ...
    if (insertError) {
      ...
      throw new Error(
        `Failed to save VAPI configuration in fallback: ${insertError.message}`,
      );
    }
...
export async function testVapiConfigurationAction(
  config: z.infer<typeof vapiConfigSchema>,
) {
  try {
    ...
    const result = await saveVapiConfigWithFallback(
      serviceClient,
      userId,
      config,
    );
    ...
    if (!result.success) {
      ...
      if (
        ...
        if (errorCode === "23505") {
          console.error(
            "Duplicate key error detected. This suggests the database needs the unique constraint on user_id.",
          );
          console.error(
            "Please run the FIX_VAPI_CONFIGS_UNIQUE_CONSTRAINT.sql script in your database.",
          );
          throw new Error(
            "Database constraint issue: The vapi_configs table needs a unique constraint on user_id. Please run the database fix script.",
          );
        }
      }
      throw result.error;
    ...
    console.log("VAPI configuration saved successfully:", {
      isNew,
    ...
    return {
      success: true,
      message: isNew
        ? "VAPI configuration created successfully"
        : "VAPI configuration updated successfully",
      config: {
        id: savedConfig.id,
        user_id: userId,
    ...
    throw new Error(
      `VAPI configuration test failed: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}

// Create test property
export async function createTestPropertyAction(
  propertyData: z.infer<typeof testPropertySchema>,
) {
  try {
    const userId = await getUserId();
    const serviceClient = createServiceRoleClient();
    ...
    if (propertyError) {
      throw new Error(
        `Failed to create test property: ${propertyError.message}`,
      );
    }
    ...
    throw new Error(
      `Property creation failed: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
...
export async function getVapiConfigurationAction() {
  try {
    ...
    throw new Error(
      `Failed to get VAPI configuration: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
...
export async function getIntegrationStatusAction() {
  try {
    ...
    throw new Error(
      `Failed to get integration status: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
  ...
  try {
    ...
    if (existingUser) {
      ...
      return {
        success: true,
        message:
          "Database connection test successful - user exists and VAPI config saved",
    ...
    try {
      ...
      if (userId) {
        ...
        return {
          success: true,
          message:
            "Database connection test successful - user and VAPI config created",
          userId: userId,
          configId: config.id,
        };
      }
