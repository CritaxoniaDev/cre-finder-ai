...
async function setupVAPITestConfig() {
  try {
    ...
    const { error } = await supabase.from("vapi_configs").insert({
...
async function setupTwilioTestConfig() {
  try {
    ...
    const { error } = await supabase.from("twilio_configs").insert({
...
async function storeVAPICredentials(credentials: {
  apiKey: string;
  assistantId: string;
}) {
...
async function storeTwilioCredentials(credentials: {
  accountSid: string;
  authToken: string;
  phoneNumber: string;
}) {
