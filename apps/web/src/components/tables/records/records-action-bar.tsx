...
export function RecordsTableActionBar({
  ...
  // New: handle send campaign
  const onSendCampaign = React.useCallback(() => {
    setDialogOpen(true);
  }, []);

  // Prepare selected properties for dialog
  const selectedProperties = rows.map((row) => row.original);
  ...
  return (
    <>
      <DataTableActionBar table={table} visible={rows.length > 0}>
        ...
        <div className="flex items-center gap-1.5">
          ...
          >
            ...
            <span role="img" aria-label="Send">📣</span> Send Campaign
          </DataTableActionBarAction>
      ...
      {/* PropertyDialog for campaign */}
      <PropertyDialog
        isOpen={dialogOpen}
        onClose={() => setDialogOpen(false)}
        propertiesArray={selectedProperties}
