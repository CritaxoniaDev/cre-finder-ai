...
export function LicensePreview() {
  ...
  const { data: availability } = useSuspenseQuery(
    trpc.licenses.checkAvailability.queryOptions(
      {
        locations,
        assetTypeSlug: asset_type!,
      },
      {
        enabled: !!asset_type && !!locations?.length,
      },
    ),
  );

  const { data: assetTypeData } = useSuspenseQuery(
    trpc.assetTypes.get.queryOptions(
      {
        slug: asset_type!,
      },
      {
  ...
  const { data: propertyCountsData } = useSuspenseQuery(
  ...
  const hasValidResults = propertyCountsData.meta.total > 0;
  const isAnyLocationTaken = availability.meta.isAnyTaken || false;
    ...
    <div className="absolute inset-0 flex flex-col justify-center z-40 gap-4 px-4 sm:px-0">
      ...
      <div className="mx-auto w-full max-w-lg px-2 sm:px-0">
        ...
        <div className="mb-6 sm:mb-8">
          <LocationPreviewList locationCounts={propertyCountsData.data} />
        </div>
              ...
              <div className="flex items-start gap-2 sm:gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground leading-relaxed">
                    One or more of the selected locations are already licensed
                    by another user. Please select different locations or refine
              ...
              <div className="flex items-start gap-2 sm:gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground leading-relaxed">
                    ...
                    criteria in the selected locations. Please adjust your
                    search filters above or select different locations to
        ...
        <div className="flex flex-col sm:flex-row sm:justify-center gap-3">
          ...
          {!isAnyLocationTaken && <GetAccessButton />}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-2 max-h-56 sm:max-h-64 overflow-y-auto">
      ...
      {locationCounts.map(({ resultCount, formattedLocation, internalId }) => (
        <div
          key={internalId}
          className="flex items-center justify-between py-2.5 sm:py-3 px-3 sm:px-4 rounded-md bg-muted/30 border border-border/50"
        >
