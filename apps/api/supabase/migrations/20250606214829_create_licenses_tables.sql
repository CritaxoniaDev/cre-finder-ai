...
CREATE TABLE
    location_licenses (
        ...
        WITH
            TIME ZONE, -- When this location license expires (end of current billing period)
            is_active BOOLEAN DEFAULT true,
            created_at TIMESTAMP
        WITH
            TIME ZONE DEFAULT CURRENT_TIMESTAMP,
        ...
        WITH
            TIME ZONE DEFAULT CURRENT_TIMESTAMP,
            UNIQUE (asset_license_id, location_internal_id)
    );
...
CREATE TRIGGER update_asset_licenses_updated_at BEFORE
UPDATE ON asset_licenses FOR EACH ROW EXECUTE FUNCTION update_updated_at ();

CREATE TRIGGER update_location_licenses_updated_at BEFORE
UPDATE ON location_licenses FOR EACH ROW EXECUTE FUNCTION update_updated_at ();
...
CREATE
OR REPLACE VIEW user_licenses_by_asset_type AS
SELECT
    al.user_id,
    al.asset_type_slug,
